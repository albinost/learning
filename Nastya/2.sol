/*
Дать понятие массива, перечислить виды массивов в Solidity.
Написать контракт для добавления и удаления элементов. 
Плюс возможность удаления N-го элемента.
Рассмотреть все варианты создания массивов
и их функциональное применение.

https://ropsten.etherscan.io/address/0x1E20912D529EBD23FfF9F0E2bC64eAde5F761406
*/
...
*/
    ...
    function inputDyn (uint[] calldata data) public {     //ввод динамического массива
        dynamic = data;
    }
    
    function inputFix (uint[] calldata data) public {      //ввод фиксированного массива
    ...
    function inputMix (string[] calldata data) public {     //ввод смешанного массива строк
    ...
    function input2D(uint[][] calldata data) public view returns(uint[][] memory) {
    ...
    function changeDynFix(uint data,uint index,uint number) public {       //изменение элементов двух массивов
        if (numbe r== 0) {                                            //динамического и фиксированного
            require(index < dynamic.length,"Wrong index");
            dynamic[index]= data;
        } else if (number == 1) {
            require(index < fixeed.length, "Wrong index");
            fixeed[index] = data;
        }
    }
    
    function change2D(string memory data,uint i,uint j) public {         //изменение двумерного массива строк
        require((j < 2) && (i < mixed.length), "Wrong index");
        mixed[i][j] = data;
    }    
    ...
    function opredelitel2X2(int256[][] calldata data) public view returns(int opredelitel)  {
    ...
    function removeMix(uint i,uint j) public {       //удаление элемента без перетасовки
        require((j < 2) && (i < mixed.length), "Wrong 2D index");
        delete mixed[i][j];
    }
    ...
    function deleteMix() public {       //удаляет смешанный массив
       delete mixed;
    }
 
    ///геттеры
    function getDynam() public view returns (uint[] memory) {
         return dynamic;
    } 
    ...
    function getFix() public view returns (uint[5] memory) {
     ...
     function getMix() public view returns (string[2][] memory) {    //для вывода массива целиком 
         return mixed;
    }
    
    function getMixLen() public view returns (uint){    
         return mixed.length;
    }
}
